...
export const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const [isLoginView, setIsLoginView] = useState(true); // Toggle antara Login dan Daftar
  ...
  const handleAuthSuccess = async (user: any) => {
      ...
      if (existingUser) {
        ...
        // BELUM TERDAFTAR DI DB (Tapi sudah lolos Auth): Arahkan ke form lengkapi profil
  ...
  const handleAuthError = (error: any) => {
      ...
      if (error.code === 'auth/unauthorized-domain') {
        ...
        setAuthError("Akses Database Ditolak. Pastikan Firestore Rules benar.");
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row font-sans">
      ...
      <div className="md:w-1/2 bg-white flex items-center justify-center p-6 md:p-12 relative overflow-y-auto">
          ...
          {domainError && (
            <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-5 shadow-lg animate-fade-in relative overflow-hidden">
                ...
                <p className="text-sm text-gray-700 mb-4">
                   Domain aplikasi ini <b>belum didaftarkan</b> di Google/Firebase Console.
                </p>
                <div className="bg-white border rounded p-3 flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => {navigator.clipboard.writeText(domainError); alert("Domain disalin!");}} className="text-xs bg-gray-100 px-3 py-1.5 rounded border hover:bg-gray-200 font-medium">Salin</button>
                </div>
                <button onClick={switchToOfflineMode} className="w-full bg-red-600 text-white py-2 rounded text-sm font-bold hover:bg-red-700 shadow-md">Lewati (Mode Demo)</button>
            </div>
          )}

          {authError && !domainError && (
             <div className="mb-6 bg-orange-50 text-orange-800 p-4 rounded-lg border border-orange-200 text-sm flex gap-3 items-start animate-fade-in">
          ...
          {!isRegistering ? (
             <div className="animate-fade-in space-y-6">
                <div className="text-center">
                   ...
                   <p className="text-gray-500">{isLoginView ? 'Masuk ke akun Anda' : 'Daftar akun baru'}</p>
                </div>
                ...
                <form onSubmit={handleEmailAuth} className="space-y-4">
                    <div>
                        ...
                        <input 
                            type="email" 
                            required 
                            className="w-full border-gray-300 border p-3 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none"
                    ...
                    <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full bg-green-600 text-white font-bold py-3 rounded-xl hover:bg-green-700 shadow-lg shadow-green-100 transition-all"
                    >
                ...
                <div className="text-center text-sm">
                    {isLoginView ? (
                        <p className="text-gray-600">
                            Belum punya akun?{' '}
                            <button onClick={() => setIsLoginView(false)} className="text-green-600 font-bold hover:underline">Daftar sekarang</button>
                        </p>
                    ) : (
                        <p className="text-gray-600">
                            Sudah punya akun?{' '}
                            <button onClick={() => setIsLoginView(true)} className="text-green-600 font-bold hover:underline">Masuk di sini</button>
                        </p>
                    )}
                </div>
                ...
                <div className="relative py-2">
                   ...
                   <div className="relative flex justify-center text-sm"><span className="px-2 bg-white text-gray-400">Atau masuk dengan</span></div>
                </div>
                ...
                <div className="text-center mt-4">
                   <button onClick={switchToOfflineMode} className="text-xs text-gray-400 hover:text-gray-600 underline">
